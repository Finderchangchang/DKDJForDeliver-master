...
public class CommonViewHolder {
    ...
    //设置哪个控件的可见性
    public CommonViewHolder SetState(int viewId, boolean state) {
        LinearLayout view = null;
        Button button = null;
        if (viewId == R.id.left_btn) {//设置接单按钮的可见性
            button = getView(viewId);
            setvisibility(button, state);
        } else {//设置图片布局的可见性
            view = getView(viewId);
            setvisibility(view, state);
        }
    ...
    /**设置控件可见性*/
    public void setvisibility(View view, boolean state) {
        if (state) {
     ...
     * 设置picasso加载图片
     */
    public CommonViewHolder instanImageview(int viewId, Context context, String imgstr) {
        ImageView view = getView(viewId);
        Picasso.with(context).load(imgstr).into(view);
        return this;
    }
